 ...
 * @param {Object} optOptions plain object of options for exec function
 * @param  {boolean=}   optSilent Suppress stdout messages.
 * @return {void}
 */
helpers.executeCommand = function executeCommand( command, done, optOptions, optSilent ) {
  ...
  if (optOptions) {
    exec(command, optOptions, execCB);
  } else {
    exec(command, execCB);
  }
};
 ...
 */
helpers.runCommands = function runCommands( commands, cb, optOptions, optSilent) {
